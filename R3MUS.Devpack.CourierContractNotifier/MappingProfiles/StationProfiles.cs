...
{
    public class StationProfiles : Profile
    {
        public StationProfiles()
        {
            CreateMap<Station, Endpoint>()
                .ForMember(dest => dest.Id, opt => opt.MapFrom(src => src.Id))
                .ForMember(dest => dest.Name, opt => opt.MapFrom(src => src.Name))
                .ForMember(dest => dest.System, opt => opt.MapFrom(src => new Entity() { Id = src.Id }))
                .ForMember(dest => dest.EndpointType, opt => opt.MapFrom(src => EndpointTypes.Station));
        }
    }
}
